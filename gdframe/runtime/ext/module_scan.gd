extends RefCounted
## 扫描 [member GDFrameConfig.EXT_ADDONS_DIR] 下 [code]gdframe_*/module.gd[/code]。
...
static func load_modules() -> Array[Script]:
	...
	_collect_modules(GDFrameConfig.EXT_ADDONS_DIR, out)
...
static func _try_append_module(dir_path: String, dir_name: String, out: Array[Script]) -> void:
	if not GDFrameConfig.is_ext_addon_dir_name(dir_name):
		return
	var module_path: String = dir_path.path_join(dir_name).path_join("module.gd")
...
static func _collect_from_listing(
	...
	for entry: String in listed:
		...
		if sub.begins_with("."):
			continue
		_try_append_module(dir_path, sub, out)
...
static func _collect_with_dir_access(dir_path: String, out: Array[Script]) -> void:
	...
	for entry: String in dir.get_directories():
		if entry.begins_with("."):
			continue
		_try_append_module(dir_path, entry, out)
