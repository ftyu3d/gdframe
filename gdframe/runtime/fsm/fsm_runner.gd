...
func _rebuild_registry_index() -> void:
	...
	for reg_path: String in FSMScan.collect_fsm_registry_paths(GDFrameConfig.FSM_ROOT):
		var machine_id: String = reg_path.get_file().get_basename().trim_suffix("_registry")
		if machine_id.is_empty():
			continue
		_registry_path_by_machine[StringName(machine_id)] = reg_path
...
func _ensure_machine_loaded(machine_id: StringName, force_reload: bool = false) -> bool:
	...
	if reg_path.is_empty():
		push_error("GDFrame FSM: 未找到状态机 %s（检查 %s 与 Registry 命名）" % [machine_id, GDFrameConfig.FSM_ROOT])
		return false
	var reg: GDFrameFSMRegistryBase = _load_registry_from_path(reg_path, force_reload)
	if reg == null:
		return false
	return _register_registry(machine_id, reg, reg_path)
...
func _register_registry(
	...
	if String(mid_sn).is_empty():
		push_error("GDFrame FSM: machine_id 为空，跳过 %s" % reg_path)
		return false
	if mid_sn != index_mid:
		push_warning(
	...
	if key_sn.is_empty():
		push_error("GDFrame FSM: 状态列表为空，跳过 %s" % reg_path)
		return false
