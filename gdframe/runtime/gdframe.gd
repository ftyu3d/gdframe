...
extends "res://gdframe_project/facade.gd"
...
func _ready() -> void:
	...
	_settings = SettingsManager.new(_save_manager)
...
func _input(event: InputEvent) -> void:
	...
	if nav_action == GDFrameUINav.NavPress.NONE:
		return
	if not input_should_allow_nav_focus():
		return
...
## 预加载：实例化并执行 [_on_init]（内部可 [code]await[/code]）。
## 用法：[code]await GDFrame.ui_preload(ui_id)[/code]。
## 打开前必须先预加载；仅预加载需要 [code]await[/code]（[code]ui_open[/code] 仍为同步）。
func ui_preload(ui_id: StringName) -> void:
	await _ui.preload_ui(ui_id)


## 同步打开已预加载的 UI（勿 [code]await[/code]）。
...
## 关闭 [param close_id] 并打开 [param open_id]；关闭 [param open_id] 时自动重新打开 [param close_id] 并恢复离开时的聚焦项。连续 replace 时不会把更早一页提前拉回。
...
## [param reopen_replaced] 为 [code]false[/code] 时，即使该页是 [method ui_open_replace] 打开的也不再打开上一页。
## 返回 [code]false[/code] 表示拒绝关闭（[code]_on_close[/code] 返回了 [code]false[/code]）。
func ui_close(ui_id: StringName, cache: bool = true, reopen_replaced: bool = true) -> bool:
	return _ui.close(ui_id, cache, reopen_replaced)
...
## 根据物理 [InputEvent] 更新当前设备。[InputEventMouseMotion] 直接跳过（悬停不切换设备）。
...
## 切换至 [param device_kind] 时是否应隐藏鼠标（见 [member GDFrameConfig.keyboard_nav_focus]）。
func input_should_hide_cursor(
	device_kind: GDFrameInputEventDevice.DeviceKind,
) -> bool:
	return GDFrameInputEventDevice.should_hide_cursor(
		device_kind,
		_config.keyboard_nav_focus,
	)


## 当前设备是否允许给导航项抢 GUI 焦点（见 [member GDFrameConfig.keyboard_nav_focus]）。
func input_should_allow_nav_focus() -> bool:
	return GDFrameInputEventDevice.should_allow_nav_focus(
...
func pause_pop_deferred(reason: StringName) -> void:
	call_deferred(&"pause_pop", reason)
...
## 暂停 / 恢复 BGM、SFX，并发出 [signal signal_gdframe_audio_paused]；[method audio_play_ui_sfx] 不受拦截。
