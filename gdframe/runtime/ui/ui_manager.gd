...
func open(ui_id: StringName, data: Variant = null, layer: int = -1) -> Control:
	...
	var already_open: bool = is_open(ui_id)
...
func close(ui_id: StringName, cache: bool = true, reopen_replaced: bool = true) -> bool:
	...
	if _frame != null:
		_frame.call_deferred(&"_nav_focus_restore", nav_restore)
	if reopen_replaced and _nav_reopen_on_close.has(ui_id):
...
func get_cached(ui_id: StringName) -> Control:
...
func notify_locale() -> void:
	for ui_id: StringName in _cache.keys():
		var ui: GDFrameUIBase = _get_cached_ui(ui_id)
		if ui == null or not ui.visible:
			continue
		ui._on_locale_changed()
...
func restore_nav_return_focus(restore_after_close: Dictionary) -> void:
	...
	if parent == null:
		return
	var restore_path: String = _nav_restore_control_path(restore_after_close)
	if restore_path.is_empty():
		return
	parent.ui_nav_apply_focus_by_path(restore_path)


## 处理 [code]ui_cancel[/code]（树暂停时 Control 可能收不到该动作）。
...
func _stack_remove(ui_id: StringName) -> void:
	...
	for i: int in range(idx, _stack.size()):
		_stack_index[_stack[i]] = i
...
func _set_visible(ui: Control, p_visible: bool) -> void:
	if is_instance_valid(ui):
		ui.visible = p_visible
...
func _instantiate_dim() -> Control:
	var path: String = GDFrameConfig.DIM_SCENE_PATH
	var msg_missing: String = "GDFrame UI: 缺少遮罩场景 %s（请重载 GDFrame 插件）。" % path
	assert(ResourceLoader.exists(path), msg_missing)
	var scene: PackedScene = load(path) as PackedScene
	var msg_load: String = "GDFrame UI: 无法加载遮罩场景 %s" % path
	assert(scene != null, msg_load)
	var dim: Control = scene.instantiate() as Control
	var msg_root: String = "GDFrame UI: 遮罩场景根节点须为 Control — %s" % path
	assert(dim != null, msg_root)
	return dim


func _notify_dim_visible(p_visible: bool, ui: Control) -> void:
	if _dim == null:
		return
	...
	elif _dim.has_method(&"on_dim_hide"):
...
func _nav_resolve_restore_parent(restore_after_close: Dictionary) -> GDFrameUIBase:
	...
	if parent_id_v == null or String(parent_id_v) == "":
		return null
	var parent: GDFrameUIBase = _get_cached_ui(StringName(String(parent_id_v)))
	if not is_instance_valid(parent):
		return null
	return parent

func _nav_restore_control_path(restore_after_close: Dictionary) -> String:
	var path_v: Variant = restore_after_close.get("control_path", "")
	if path_v is String:
		return String(path_v)
	return ""
...
## 打开子 UI 时写入还焦票据（[code]parent_id[/code] + [code]control_path[/code]）。
...
func _nav_bind_return_focus_ticket(
	...
	if return_focus == null or not parent.is_ancestor_of(return_focus):
		return
	parent.ui_nav_remember_focus(return_focus)
	_nav_return_focus[opened_ui_id] = {
		"parent_id": String(parent_id),
		"control_path": str(parent.get_path_to(return_focus)),
	}
...
func _nav_lock_stack_below(opened_ui_id: StringName) -> void:
	...
	for i: int in range(top_idx - 1, -1, -1):
		...
		var below_ui: GDFrameUIBase = _get_cached_ui(below_id)
		if not is_instance_valid(below_ui) or not below_ui.visible:
			continue
...
func _nav_release_gui_focus_from_locked(opened_ui_id: StringName) -> void:
	...
	if owner == null:
		return
	for below_id: Variant in locked_ids:
		var below_ui: GDFrameUIBase = _get_cached_ui(below_id as StringName)
		if not is_instance_valid(below_ui):
			continue
...
func _nav_ensure_parent_locked(opened_ui_id: StringName, nav_parent: GDFrameUIBase) -> void:
	if not is_instance_valid(nav_parent) or not nav_parent.visible:
		return
	# 父级可能来自栈外浮层（ui_nav_participates=false），不可挂起。
...
func _nav_unlock_for_close(closed_ui_id: StringName) -> void:
	var locked_ids: Array = _nav_locked_by.get(closed_ui_id, []) as Array
...
func _nav_clear_stale_suspend_on_stack() -> void:
	if _stack.is_empty():
		return
	var top_id: StringName = _stack[_stack.size() - 1]
	var top: GDFrameUIBase = _get_cached_ui(top_id)
	if not is_instance_valid(top):
		return
	if not top.ui_nav_can_receive_focus():
		return
...
func _nav_unsuspend_if_stack_top(ui: GDFrameUIBase, stack_index: int) -> bool:
	...
	if stack_index != _stack.size() - 1:
		return false
	ui.ui_nav_force_ready_for_input()
	return true
