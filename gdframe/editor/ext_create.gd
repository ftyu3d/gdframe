...
static func build_files(module_id: String, display_name: String, template: int) -> Dictionary:
	...
	files["plugin.cfg"] = _plugin_cfg_text(display_name)
	files["plugin.gd"] = _plugin_gd_text(display_name)
...
static func _plugin_cfg_text(display_name: String) -> String:
	return (
		"[plugin]\n\n"
		+ 'name="GDFrame %s"\n' % display_name
		+ 'description="GDFrame 扩展：%s"\n' % display_name
		+ 'author=""\n'
		+ 'version="0.1.0"\n'
		+ 'script="plugin.gd"\n'
	)
...
static func _plugin_gd_text(display_name: String) -> String:
	return (
		"@tool\n"
		+ "extends EditorPlugin\n"
		+ "## GDFrame 扩展插件入口（%s）。运行时由 [code]module.gd[/code] 注册。\n"
	) % display_name
...
static func _build_readme(module_id: String, display_name: String, template: int) -> String:
	...
	return (
		"# %s\n\n"
		+ "GDFrame 扩展模块 `%s`（插件目录 `addons/gdframe_%s/`）。\n\n"
		+ "## API\n\n"
		+ "%s\n"
	) % [display_name, module_id, module_id, api_hint]
