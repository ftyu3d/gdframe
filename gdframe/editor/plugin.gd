...
var _label_general_status: Label
...
var _external_editor_setup_dialog: AcceptDialog
...
var _version_confirm_dialog: ConfirmationDialog
var _pending_delete_ext_dir: String = ""
var _pending_version_confirm: Dictionary = {}
...
var _label_create_ui_status: Label
var _ui_filter_edit: LineEdit
...
var _label_rename_ui_target: Label
...
var _label_source_url: Label
var _label_local_version: Label
...
var _asset_fs_sync_refresh_addons: bool = false
...
func _enter_tree() -> void:
	...
	call_deferred("_regenerate_gdc_registry")
...
func _set_dock_status(slot: String, text: String) -> void:
	...
	match slot:
		"general":
			label = _label_general_status
		"ext":
			label = _label_ext_status
		"ui":
			label = _label_create_ui_status
		"fsm":
			label = _label_fsm_status
		"pool":
			label = _label_pool_status
		"update":
			label = _label_status
...
func _build_dock() -> void:
	...
	_label_rename_ui_target = Label.new()
	_label_rename_ui_target.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	rename_box.add_child(_label_rename_ui_target)
	...
	_delete_all_saves_dialog.dialog_text = (
		"将永久删除 user://gdframe 下的全部内容（profile、slots、账号等）。\n此操作不可撤销，是否继续？"
	)
	...
	_external_editor_setup_dialog = AcceptDialog.new()
	_configure_plugin_modal_dialog(_external_editor_setup_dialog)
	_external_editor_setup_dialog.title = "需要配置外部编辑器"
	_external_editor_setup_dialog.dialog_text = (
		"未能自动找到 Cursor / VS Code 可执行文件。\n\n"
		+ "请打开：编辑器 → 编辑器设置 → 文本编辑器 → 外部\n"
		+ "填写「执行路径」，勾选「使用外部编辑器」，然后重试。"
	)
	_external_editor_setup_dialog.ok_button_text = "知道了"
	_dock.add_child(_external_editor_setup_dialog)
	...
	_version_confirm_dialog = ConfirmationDialog.new()
	_configure_plugin_modal_dialog(_version_confirm_dialog)
	_version_confirm_dialog.title = "确认版本"
	_version_confirm_dialog.confirmed.connect(_on_version_confirm_confirmed)
	_version_confirm_dialog.canceled.connect(_on_version_confirm_canceled)
	_dock.add_child(_version_confirm_dialog)
...
func _call_editor_gen(gen: Script, method: StringName) -> Variant:
...
func _run_ext_facade_generate() -> Dictionary:
	var raw: Variant = _call_editor_gen(ExtFacadeGen, &"generate_all")
...
func _call_profile_settings_generate() -> Dictionary:
	var raw: Variant = _call_editor_gen(ProfileSettingsGen, &"generate_all")
	if raw is Dictionary:
		return raw
	return {
		"ok": false,
...
func _sync_result_constants() -> Dictionary:
	_ensure_generated_dir()
	var raw: Variant = _call_editor_gen(ResultGen, &"generate_all")
	if raw is Dictionary:
		return raw
...
func _sync_signals_script(ensure_sync_fs: bool = true) -> Dictionary:
	...
	var raw: Variant = _call_editor_gen(SignalsGen, &"generate_all")
	if raw is Dictionary:
		return raw
...
func _update_editor_filesystem(res_path: String) -> void:
	if res_path.is_empty():
		return
	# 新建资源时仅 update_file 不足以刷新文件系统 Dock，需 scan。
...
func _reset_asset_filesystem_sync_state() -> void:
	...
	_asset_fs_sync_refresh_addons = false
...
func _on_asset_filesystem_scan_done() -> void:
	...
	var refresh_addons: bool = _asset_fs_sync_refresh_addons
	_asset_fs_sync_paths = PackedStringArray()
	_asset_fs_sync_refresh_ui = false
	_asset_fs_sync_refresh_addons = false
...
func _build_general_page() -> Control:
	...
	_save_fmt_opt.tooltip_text = (
		"与 %s 中 save_file_format 一致；profile 与 slots 均使用该格式。切换时将现有存档转换为新格式。"
		% GDFrameConfig.PATH
	)
	...
	row_saves.add_child(_general_tool_button(
		"打开存档路径",
		"在文件管理器中打开 %s（profile、slots、账号等）；不存在则先创建。" % GDFrameSaveManager.USER_ROOT_URL,
	...
	_label_general_status = _dock_status_label()
	panel.add_child(_label_general_status)
...
func _on_open_external_editor_project_folder() -> void:
	_set_dock_status("general", "")
	if not _ensure_external_editor_ready_for_project():
		return
	if _launch_external_editor_project():
		_set_dock_status("general", "已请求打开: %s" % _external_editor_workspace_global_path())
...
const _EXTERNAL_EDITOR_USE: String = "text_editor/external/use_external_editor"
const _EXTERNAL_EDITOR_PATH: String = "text_editor/external/exec_path"
const _EXTERNAL_EDITOR_FLAGS: String = "text_editor/external/exec_flags"
const _EXTERNAL_EDITOR_DEFAULT_FLAGS: String = "{project}"


func _external_editor_enabled() -> bool:
	var settings: EditorSettings = get_editor_interface().get_editor_settings()
	return bool(settings.get_setting(_EXTERNAL_EDITOR_USE))


func _external_editor_exec_path_setting() -> String:
	var settings: EditorSettings = get_editor_interface().get_editor_settings()
	return str(settings.get_setting(_EXTERNAL_EDITOR_PATH)).strip_edges()
...
func _external_editor_exec_path() -> String:
	...
	if not _external_editor_enabled():
		...
		_external_editor_exec_cache = _external_editor_exec_path_setting()
	_external_editor_exec_cache_ready = true
	return _external_editor_exec_cache


func _invalidate_external_editor_exec_cache() -> void:
	_external_editor_exec_cache_ready = false
	_external_editor_exec_cache = ""


func _is_external_editor_exec_valid(path: String) -> bool:
	var trimmed: String = path.strip_edges()
	if trimmed.is_empty():
		return false
	return FileAccess.file_exists(trimmed)


func _ensure_external_editor_ready_for_project() -> bool:
	var settings: EditorSettings = get_editor_interface().get_editor_settings()
	var path: String = _external_editor_exec_path_setting()
	var notes: PackedStringArray = PackedStringArray()

	if not _is_external_editor_exec_valid(path):
		var detected: String = _detect_external_editor_exec_path()
		if detected.is_empty():
			if _external_editor_setup_dialog != null:
				_external_editor_setup_dialog.popup_centered()
			_set_dock_status(
				"general",
				"未能自动配置外部编辑器，请手动设置执行路径。",
			)
			return false
		settings.set_setting(_EXTERNAL_EDITOR_PATH, detected)
		path = detected
		notes.append("已自动填入执行路径")
		var flags: String = str(settings.get_setting(_EXTERNAL_EDITOR_FLAGS)).strip_edges()
		if flags.is_empty():
			settings.set_setting(_EXTERNAL_EDITOR_FLAGS, _EXTERNAL_EDITOR_DEFAULT_FLAGS)
			notes.append("已设置默认启动参数 {project}")

	if not bool(settings.get_setting(_EXTERNAL_EDITOR_USE)):
		settings.set_setting(_EXTERNAL_EDITOR_USE, true)
		notes.append("已启用外部编辑器")

	_invalidate_external_editor_exec_cache()
	if not notes.is_empty():
		_set_dock_status("general", "%s。" % "；".join(notes))
	return true


func _detect_external_editor_exec_path() -> String:
	var candidates: PackedStringArray = _external_editor_candidate_paths()
	for candidate: String in candidates:
		if _is_external_editor_exec_valid(candidate):
			return candidate
	return ""


func _external_editor_candidate_paths() -> PackedStringArray:
	var out: PackedStringArray = PackedStringArray()
	var os_name: String = OS.get_name()
	match os_name:
		"Windows":
			var local_app: String = OS.get_environment("LOCALAPPDATA").replace("\\", "/")
			var prog_files: String = OS.get_environment("ProgramFiles").replace("\\", "/")
			var prog_files_x86: String = OS.get_environment("ProgramFiles(x86)").replace("\\", "/")
			for base: String in [local_app, prog_files, prog_files_x86]:
				if base.is_empty():
					continue
				out.append("%s/Programs/cursor/Cursor.exe" % base)
				out.append("%s/Programs/Cursor/Cursor.exe" % base)
				out.append("%s/cursor/Cursor.exe" % base)
				out.append("%s/Cursor/Cursor.exe" % base)
				out.append("%s/Programs/Microsoft VS Code/Code.exe" % base)
				out.append("%s/Microsoft VS Code/Code.exe" % base)
			out.append_array(_which_command_paths(["cursor.cmd", "cursor.exe", "cursor", "code.cmd", "code.exe", "code"]))
		"macOS":
			var home: String = OS.get_environment("HOME")
			var app_roots: PackedStringArray = PackedStringArray(["/Applications"])
			if not home.is_empty():
				app_roots.append("%s/Applications" % home)
			for root: String in app_roots:
				out.append("%s/Cursor.app/Contents/MacOS/Cursor" % root)
				out.append("%s/Cursor.app/Contents/Resources/app/bin/cursor" % root)
				out.append(
					"%s/Visual Studio Code.app/Contents/Resources/app/bin/code" % root
				)
				out.append("%s/Visual Studio Code.app/Contents/MacOS/Electron" % root)
				out.append("%s/Code.app/Contents/Resources/app/bin/code" % root)
				out.append("%s/Code.app/Contents/MacOS/Electron" % root)
			out.append_array(_which_command_paths(["cursor", "code"]))
		_:
			var home_unix: String = OS.get_environment("HOME")
			if not home_unix.is_empty():
				out.append("%s/.local/bin/cursor" % home_unix)
				out.append("%s/.local/bin/code" % home_unix)
			out.append("/usr/bin/cursor")
			out.append("/usr/bin/code")
			out.append("/usr/share/cursor/cursor")
			out.append("/usr/share/code/bin/code")
			out.append("/snap/bin/code")
			out.append_array(_which_command_paths(["cursor", "code"]))
	return out


func _which_command_paths(names: PackedStringArray) -> PackedStringArray:
	var found: PackedStringArray = PackedStringArray()
	var finder: String = "where" if OS.get_name() == "Windows" else "which"
	for name: String in names:
		var output: Array = []
		var exit_code: int = OS.execute(finder, [name], output, true, false)
		if exit_code != 0 or output.is_empty():
			continue
		for line_variant: Variant in output:
			for raw_line: String in str(line_variant).split("\n"):
				var line: String = raw_line.strip_edges().replace("\\", "/")
				if line.is_empty():
					continue
				if _is_external_editor_exec_valid(line) and not found.has(line):
					found.append(line)
	return found
...
func _build_external_editor_args_project_only() -> PackedStringArray:
	var settings: EditorSettings = get_editor_interface().get_editor_settings()
	var flags: String = str(settings.get_setting(_EXTERNAL_EDITOR_FLAGS))
	var project_dir: String = _external_editor_workspace_global_path()
	var expanded: String
...
## 外部编辑器工作区目录：优先取工程根之上最近的 Git 仓库根。
func _external_editor_workspace_global_path() -> String:
	var start: String = _project_root_global_path()
	var dir: String = start
	for _i: int in range(8):
		if _directory_has_git_marker(dir):
			return dir
		var parent: String = dir.get_base_dir().rstrip("/\\")
...
func _directory_has_git_marker(dir: String) -> bool:
	var git_dir: String = dir.path_join(".git")
...
func _launch_external_editor_project() -> bool:
	...
	if exec_path.is_empty() or not _is_external_editor_exec_valid(str(exec_path)):
		_set_dock_status("general", "外部编辑器可执行文件路径无效。")
		return false
...
func _build_create_ui_page() -> Control:
	...
	_label_create_ui_status = _dock_status_label()
	panel.add_child(_label_create_ui_status)
	_set_dock_status("ui", "")
	...
	refresh_btn.tooltip_text = "重新扫描 UI 并更新 GDFrameConstants"
	refresh_btn.size_flags_horizontal = Control.SIZE_SHRINK_BEGIN
	refresh_btn.pressed.connect(_on_ui_refresh_list_pressed)
	manage_row.add_child(refresh_btn)

	_ui_filter_edit = LineEdit.new()
	_ui_filter_edit.placeholder_text = "搜索（模糊匹配，不区分大小写）"
	_ui_filter_edit.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_ui_filter_edit.clear_button_enabled = true
	_ui_filter_edit.text_changed.connect(_on_ui_filter_changed)
	manage_row.add_child(_ui_filter_edit)
...
func _on_ui_refresh_list_pressed() -> void:
	_after_asset_tree_change(true, false, false)


func _on_ui_filter_changed(_new_text: String) -> void:
	_refresh_ui_manage_list()


func _ui_fuzzy_match(haystack: String, needle: String) -> bool:
	if needle.is_empty():
		return true
	var h: String = haystack.to_lower()
	var n: String = needle.to_lower()
	if h.contains(n):
		return true
	var hi: int = 0
	for ni: int in range(n.length()):
		var ch: String = n.substr(ni, 1)
...
func _refresh_ui_manage_list(scenes: Array[String] = []) -> void:
	...
	if _ui_filter_edit != null:
		filter_raw = _ui_filter_edit.text.strip_edges()
	...
	for scene_path in scenes:
		...
		if not _ui_fuzzy_match(scene_name, filter_raw) and not _ui_fuzzy_match(scene_path, filter_raw):
			continue
	...
	if scenes.is_empty():
		...
		no_match.text = "无匹配项"
		_ui_manage_list.add_child(no_match)
		_set_dock_status("ui", "状态: 0 / %d 个 UI" % scenes.size())
	else:
		if filter_raw.is_empty():
			_set_dock_status("ui", "状态: 已加载 %d 个 UI" % shown)
		else:
			_set_dock_status("ui", "状态: 显示 %d / %d 个 UI" % [shown, scenes.size()])

func _read_ui_scene_settings(scene_path: String) -> Dictionary:
	var defaults: Dictionary = {"layer": 0, "use_dim": false, "nav_participates": true}
	if not _res_file_exists(scene_path):
		return defaults
...
func _parse_ui_scene_settings_from_tscn(tscn_text: String) -> Dictionary:
	var settings: Dictionary = {"layer": 0, "use_dim": false, "nav_participates": true}
	for line: String in tscn_text.split("\n", false):
		...
		elif stripped.begins_with("ui_nav_participates = "):
			settings["nav_participates"] = (
				stripped.trim_prefix("ui_nav_participates = ").strip_edges() == "true"
			)
	return settings


func _patch_ui_scene_settings_in_tscn(
	tscn_text: String,
	layer: int,
	use_dim: bool,
	nav_participates: bool,
) -> String:
	...
	var nav_line: String = "ui_nav_participates = %s" % ("true" if nav_participates else "false")
...
func _build_ui_manage_row(
	...
	dim_check.text = "遮罩"
	dim_check.tooltip_text = "ui_use_dim，与场景根 GDFrame 分组一致"
	dim_check.button_pressed = bool(settings.get("use_dim", false))
	row.add_child(dim_check)

	var nav_check: CheckBox = CheckBox.new()
	nav_check.text = "导航"
	nav_check.tooltip_text = "ui_nav_participates：关闭后永不进入打开栈，也不锁定下层、不抢焦点"
	nav_check.button_pressed = bool(settings.get("nav_participates", true))
	row.add_child(nav_check)
	...
	var save_settings := func() -> void:
		if not _require_editor_not_playing("ui", "修改 UI 设置"):
			...
			nav_check.set_pressed_no_signal(bool(settings.get("nav_participates", true)))
			return
		var layer: int = layer_opt.selected
		var use_dim: bool = dim_check.button_pressed
		var nav_participates: bool = nav_check.button_pressed
		if (
			layer == int(settings.get("layer", 0))
			and use_dim == bool(settings.get("use_dim", false))
			and nav_participates == bool(settings.get("nav_participates", true))
		):
			return
		var err: Error = _write_ui_scene_settings(scene_path, layer, use_dim, nav_participates)
		if err != OK:
		...
		settings["nav_participates"] = nav_participates
		_update_editor_filesystem(scene_path)
		var flags: PackedStringArray = PackedStringArray()
		if use_dim:
			flags.append("遮罩")
		if not nav_participates:
			flags.append("不参与导航")
	...
	nav_check.toggled.connect(func(_pressed: bool) -> void: save_settings.call())
...
func _request_rename_ui(scene_path: String) -> void:
	...
	_label_rename_ui_target.text = "将 %s 重命名为：" % scene_name
...
func _delete_ui_bundle(scene_path: String, refresh_registry: bool = true) -> void:
	...
	_try_remove_empty_dir(GDFrameConfig.UI_ROOT_DIR, GDFrameConfig.PROJECT_ROOT)
	if refresh_registry:
		...
		elif _res_dir_exists(GDFrameConfig.UI_ROOT_DIR):
			notify_paths.append(GDFrameConfig.UI_ROOT_DIR)
		else:
			notify_paths.append(GDFrameConfig.PROJECT_ROOT)
...
func _build_ext_page() -> Control:
	...
	_label_ext_status = _dock_status_label()
	_label_ext_status.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	ext_status_row.add_child(_label_ext_status)
...
func _on_create_ext_pressed() -> void:
	...
	var ext_dir: String = GDFrameConfig.ext_addon_path(module_id)
	...
	_enable_ext_addon_plugin(module_id)
...
func _on_ext_generate_api_pressed() -> void:
	_apply_ext_facade_generate_result(_run_ext_facade_generate())
	if _label_general_status != null:
		var general_text: String = _label_general_status.text
...
func _apply_ext_index_status(ext_index: Dictionary) -> void:
	...
	for info: Dictionary in ExtManageList.build_manage_list(ext_index):
		...
		if selection_status == "patch_available":
			patch_count += 1
		elif selection_status == "eol_upgrade_required":
			eol_count += 1
...
func _prompt_ext_version_change(option: OptionButton, index: int, from_action_button: bool) -> void:
	...
	_pending_version_confirm = {
		"kind": &"ext",
		"option": option,
		"index": safe_index,
		"ext_id": ext_id,
		"target_version": target_version,
		"is_new_install": local_version.is_empty(),
	}
	_version_confirm_dialog.title = "确认扩展版本"
	_version_confirm_dialog.dialog_text = VersionSeries.confirm_dialog_text(
		kind,
		product_label,
		local_version,
		target_version,
	)
	_version_confirm_dialog.popup_centered(Vector2i(520, 160))


func _on_version_confirm_canceled() -> void:
	_pending_version_confirm.clear()


func _on_version_confirm_confirmed() -> void:
	var pending: Dictionary = _pending_version_confirm.duplicate()
	_pending_version_confirm.clear()
	var kind: StringName = pending.get("kind", &"") as StringName
	if kind == &"core":
		var core_target: String = str(pending.get("target_version", ""))
		if core_target.is_empty():
			return
		if not _require_editor_not_playing("update", "更新 GDFrame"):
			return
		await _apply_gdframe_update(core_target)
		return
	if kind != &"ext":
		return
	var option: OptionButton = pending.get("option") as OptionButton
	var index: int = int(pending.get("index", -1))
	if option == null or not is_instance_valid(option):
		return
	var ext_id: String = str(pending.get("ext_id", ""))
	var target_version: String = str(pending.get("target_version", ""))
	var is_new_install: bool = bool(pending.get("is_new_install", false))
...
func _apply_ext_version_install(ext_id: String, download_url: String, is_new_install: bool) -> void:
	...
	if ok:
		_enable_ext_addon_plugin(ext_id)
		_apply_ext_facade_generate_result(_run_ext_facade_generate())
		_after_asset_tree_change(
			false,
			false,
			false,
			PackedStringArray([
				GDFrameConfig.ext_addon_path(ext_id),
		...
		_set_zip_update_failure_status(
			"ext",
			"%s %s" % [action_label, ext_id],
			_ext_updater.get_last_http_status(),
		)


func _set_zip_update_failure_status(dock_key: String, subject: String, http_status: int) -> void:
	if http_status >= 200 and http_status < 300:
		_set_dock_status(dock_key, "状态: %s失败（ZIP 路径无效）" % subject)
	else:
		_set_dock_status(
			dock_key,
			"状态: %s失败（%s）" % [
...
func _delete_ext_bundle(ext_dir: String) -> void:
	...
	if _purge_abs_dir(ProjectSettings.globalize_path(ext_dir)):
		EditorInterface.set_plugin_enabled(dir_name, false)
		_apply_ext_facade_generate_result(_run_ext_facade_generate())
		_after_asset_tree_change(
			false,
			false,
...
func _sync_update_source_display(source: String) -> void:
	...
	if _label_source_url != null:
		_label_source_url.text = "URL: %s" % url
		_label_source_url.tooltip_text = url
...
func _build_update_page() -> Control:
	...
	_label_local_version = Label.new()
	_label_local_version.text = "当前版本: %s" % _get_local_version()
	version_row.add_child(_label_local_version)
	...
	_label_source_url = Label.new()
	_label_source_url.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_label_source_url.autowrap_mode = TextServer.AUTOWRAP_OFF
	_label_source_url.text_overrun_behavior = TextServer.OVERRUN_TRIM_ELLIPSIS
	panel.add_child(_label_source_url)
...
func _populate_supported_version_option(plugin_index: Dictionary) -> void:
	...
	for entry: Dictionary in listed:
		var ver: String = str(entry.get("version", ""))
		var label: String = ver
		if bool(entry.get("deprecated", false)):
			label = "%s（已弃用）" % ver
		_supported_version_option.add_item(label)
...
func _on_update_pressed() -> void:
	...
	if not _can_apply_selected_update(entry):
		...
		return
	var local_version: String = _get_local_version()
	var supported: Dictionary = _plugin_index_info.get("supported", {}) as Dictionary
	var kind: String = VersionSeries.upgrade_kind(local_version, target_version, supported)
	_pending_version_confirm = {
		"kind": &"core",
		"target_version": target_version,
	}
	_version_confirm_dialog.title = "确认更新 GDFrame"
	_version_confirm_dialog.dialog_text = VersionSeries.confirm_dialog_text(
		kind,
		"GDFrame",
		local_version,
...
func _apply_gdframe_update(target_version: String) -> void:
	...
	if ok:
		...
		_set_zip_update_failure_status("update", "更新", _updater.get_last_http_status())
	_sync_update_button()
...
func _build_pool_page() -> Control:
	...
	_label_pool_status = _dock_status_label()
	status_row.add_child(_label_pool_status)
...
func _build_fsm_page() -> Control:
	...
	_label_fsm_status = _dock_status_label()
	panel.add_child(_label_fsm_status)
	...
	var refresh_btn: Button = Button.new()
	refresh_btn.text = "刷新"
	refresh_btn.tooltip_text = "重新扫描状态机并更新 GDFrameConstants"
	refresh_btn.pressed.connect(_on_fsm_refresh_list_pressed)
	...
	_label_rename_fsm_machine_target = Label.new()
	_label_rename_fsm_machine_target.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	rename_machine_box.add_child(_label_rename_fsm_machine_target)
	...
	_label_rename_fsm_state_target = Label.new()
	_label_rename_fsm_state_target.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	rename_state_box.add_child(_label_rename_fsm_state_target)
...
func _on_fsm_refresh_list_pressed() -> void:
	_after_asset_tree_change(false, true, false)
...
func _build_fsm_registry_gd(fsm_id: String, states: Array[String]) -> String:
	...
	return """## Auto-generated by GDFrame — 由 Dock 维护，勿手改 state_script 的 match。
extends \"%s\"
...
func _on_fsm_rename_machine_requested() -> void:
	...
	_label_rename_fsm_machine_target.text = "将 %s 重命名为：" % sid
...
func _on_fsm_rename_state_requested(sid: String, st: String) -> void:
	...
	_label_rename_fsm_state_target.text = "将状态「%s」重命名为：" % st
...
func _delete_fsm_machine_bundle(fsm_id: String, refresh_registry: bool = true) -> void:
	...
	_try_remove_empty_dir(GDFrameConfig.FSM_ROOT, GDFrameConfig.PROJECT_ROOT)
	if refresh_registry:
		var notify_path: String = machine_dir
		if not _res_dir_exists(machine_dir):
			if _res_dir_exists(GDFrameConfig.FSM_ROOT):
				notify_path = GDFrameConfig.FSM_ROOT
			else:
				notify_path = GDFrameConfig.PROJECT_ROOT
		_after_asset_tree_change(false, true, false, PackedStringArray([notify_path]))
	if deleted_any:
		if refresh_registry:
...
func _ensure_project_root_dir() -> void:
	# 仅保证工程根存在；ui/、fsm/ 在首次创建对应资产时再递归创建，避免空目录。
	DirAccess.make_dir_recursive_absolute(
		ProjectSettings.globalize_path(GDFrameConfig.PROJECT_ROOT)
	)
	_try_remove_empty_dir(GDFrameConfig.UI_ROOT_DIR, GDFrameConfig.PROJECT_ROOT)
	_try_remove_empty_dir(GDFrameConfig.FSM_ROOT, GDFrameConfig.PROJECT_ROOT)
...
func _profile_settings_script_template() -> String:
	var raw: Variant = _call_editor_gen(ProfileSettingsGen, &"skeleton_template")
	if raw is String:
		return raw
	return ""
...
func _ensure_profile_settings_script(sync_filesystem: bool = true) -> void:
	if _res_file_exists(GDFrameConfig.PROFILE_SETTINGS_SCRIPT_PATH):
		return
	_ensure_res_dir(GDFrameConfig.PROJECT_ROOT)
...
func _dim_script_template() -> String:
	return (
		"## UI 全屏遮罩根脚本；可改场景节点或在此写逻辑。\n"
		+ "extends Control\n\n\n"
		+ "func on_dim_show(_ui_id: StringName) -> void:\n"
		+ "	pass\n\n\n"
...
func _project_scaffold_sync_paths() -> PackedStringArray:
	var paths: PackedStringArray = PackedStringArray([
		GDFrameConfig.PROJECT_ROOT,
		GDFrameConfig.GENERATED_DIR,
		GDFrameConfig.EXT_ADDONS_DIR,
	])
	for optional_dir: String in [GDFrameConfig.UI_ROOT_DIR, GDFrameConfig.FSM_ROOT]:
		if _res_dir_exists(optional_dir):
			paths.append(optional_dir)
...
func _sync_project_scaffold_filesystem() -> void:
	_asset_fs_sync_refresh_addons = true
	_sync_filesystem_after_asset_writes(_project_scaffold_sync_paths())


func _refresh_addons_in_filesystem() -> void:
	var fs: EditorFileSystem = get_editor_interface().get_resource_filesystem()
	if fs == null:
		return
	var addons_dir: String = GDFrameConfig.EXT_ADDONS_DIR
	if _res_dir_exists(addons_dir):
		fs.update_file(addons_dir)
...
func _regenerate_gdc_registry() -> void:
	...
	_ensure_ext_scaffold()
	_ensure_cursor_skills()
	_regenerate_constants_registry(false)
	_sync_project_scaffold_filesystem()


## 将 [code]editor/skills[/code] 同步到工程 [code].cursor/skills/<name>/SKILL.md[/code]（内容变化时覆盖）。
func _ensure_cursor_skills() -> void:
	const SKILLS_ROOT: String = "res://addons/gdframe/editor/skills"
	var abs_root: String = ProjectSettings.globalize_path(SKILLS_ROOT)
	...
	while not entry.is_empty():
		if dir.current_is_dir() and not entry.begins_with("."):
			var src: String = SKILLS_ROOT.path_join(entry).path_join("SKILL.md")
			if _res_file_exists(src):
				var text: String = _read_res_text(src)
				if not text.is_empty():
					var dst: String = "res://.cursor/skills".path_join(entry).path_join("SKILL.md")
					if not _res_file_exists(dst) or _read_res_text(dst) != text:
						_write_res_text(dst, text)
		entry = dir.get_next()
	dir.list_dir_end()
...
func _enable_ext_addon_plugin(module_id: String) -> void:
	if not _res_file_exists(GDFrameConfig.ext_plugin_cfg_path(module_id)):
		return
	EditorInterface.set_plugin_enabled(GDFrameConfig.ext_addon_dir_name(module_id), true)
