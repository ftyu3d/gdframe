...
1. 将 **`gdframe.zip`** 解压到工程根，或把本仓库 **`gdframe/`** 复制为 **`addons/gdframe/`**。
2. 打开 **项目 → 项目设置 → 插件**，启用 **GDFrame**。
3. 首次启用会在 **`res://gdframe_project/`** 自动生成工程目录与 `GDFrameConstants`；之后即可使用 `GDFrame` / `GDFrameConstants`。
...
| `fsm_*` | 状态机绑定与句柄 |
| `service_*` | 按 key 注册 / 取用业务对象 |
| `save_*` | 用户目录档案与槽位 `Resource` |
| `settings_*` | 窗口、垂直同步、语言（持久化到档案） |
| `audio_*` | 总线音量、BGM / SFX 播放 |
| `pause_*` | 游戏暂停栈（`get_tree().paused` + 音频暂停） |

在 Dock **扩展管理 → 生成扩展 API** 后，`GDFrame` 会挂上各扩展模块的 facade 方法；总览见 [GDFrame Ext](https://github.com/ftyu3d/gdframe-ext)（[Gitee](https://gitee.com/ftyu3d/gdframe-ext)），各模块 API 见 `addons/gdframe_<id>/editor/README.md`。
...
**生命周期**：启用后初始化存档、设置、音频与 UI 登记；UI 在首次 `ui_preload` 时加载；FSM 在首次调用 `fsm_*` 时加载。改 profile 字段后须 `save_flush()` 写盘；槽位由 `save_write_game_resource` 单独写盘。

---

## 3. 工程目录

工程根目录为 **`GDFrameConfig.PROJECT_ROOT`**（默认 `res://gdframe_project/`）。首次启用插件会自动生成；业务脚本与资源请放在此目录，**不要**放进 `addons/gdframe/`。
...
| `dim.gd` / `dim.tscn` | 全屏遮罩场景与根脚本；缺失时重载插件自动生成 |
| `facade.gd` | 项目扩展门面桩（`GDFrame` 继承此项；生成 API 在 `generated/facade_modules.gd`） |
| `ui/` | UI 场景根目录（`ui_*.tscn`；对应 `GDFrameConfig.UI_ROOT_DIR`；首次创建 UI 时生成） |
| `fsm/` | FSM 定义根目录（`fsm_*/…`；对应 `GDFrameConfig.FSM_ROOT`；首次创建状态机时生成） |

可选扩展是独立插件，目录必须为 **`addons/gdframe_<id>/`**（含 `module.gd` 与 `plugin.cfg`）。更新核心 `addons/gdframe/` 不会覆盖它们。运行时扫描 `module.gd` 并 `register`；编辑器入口由各扩展自己的 `EditorPlugin` 提供。

在 Dock **扩展管理** 安装或增删扩展后，点 **生成扩展 API** 可同步 facade、错误码与 profile 字段。

**`GDFrameConstants`**：在 **启用插件**、Dock **UI管理 / 状态机** 增删改，或点 **刷新列表 / 刷新** 后自动更新。用法示例：
...
| 字段 | 默认 | 说明 |
|------|------|------|
| `keyboard_nav_focus` | `false` | 仅影响键盘：`false` 时键盘不聚焦导航项（与鼠标/触屏相同）；`true` 时键盘可聚焦并显示环（与手柄相同）。鼠标/触屏始终不聚焦；手柄始终可聚焦并隐藏光标。触屏不强制隐藏系统光标 |
...
| `signal_gdframe_audio_paused(paused)` | `audio_set_paused` 时发出（`GDFrameExtStreamService` 入树连接） |

扩展模块可追加更多 `signal_*`（安装扩展并 **生成扩展 API** 后写入 `gdframe_signals.gd`）。

### 业务信号

在 **`res://gdframe_project/gdframe_signals.gd`** 的 **`# --- 业务自定义信号（可编辑）---`** 段声明，例如：
...
UI 放在 **`GDFrameConfig.UI_ROOT_DIR`**（含子目录），场景名 **`ui_*.tscn`**，根脚本 **`extends GDFrameUIBase`**（推荐 Dock 创建）。启动时自动扫描登记；首次 `ui_preload` 时才 `load`（`ui_open` 须先预加载）。标识用 **`GDFrameConstants.UI_*`**。

业务 UI 与风格复查见 Cursor Skills（`editor/skills/`，启用时同步到 `.cursor/skills/`）：**gdframe-ui**、**gdframe-framework**。
...
| `ui_nav_participates` | 同上 | 是否进入打开栈并参与导航（锁定下层、还焦）；为 `false` 时永不进栈，不受 `ui_close_top` / Esc 栈遍历影响 |
...
| `ui_open(ui_id, data, layer)` | 同步打开；`layer=-1` 用 `ui_default_layer`；**已打开**时刷新 `_on_show`；仅 `ui_nav_participates` 时升栈至顶 |
...
| 名称 | 说明 |
|------|------|
| `ui_default_layer` / `ui_use_dim` / `ui_nav_participates` / `resolve_use_dim` | 见 [图层与遮罩](#图层与遮罩) |
| `_on_init()` | 首次预加载；内部可 `await` |
| `_on_show(data)` | 每次 `ui_open` 之后 |
| `_on_close() -> bool` | 每次 `ui_close` 之前；返回 `false` 拦截关闭 |
| `_on_locale_changed()` | 语言切换后（所有已打开且可见的 UI） |
...
| `ui_closes_on_cancel()` | 是否响应 Esc（`ui_cancel`）关闭；框架从栈顶向下找第一个返回 `true` 的 UI |
| `ui_cancel_close()` | Esc 关闭行为（默认 `GDFrame.ui_close`） |
| `ui_nav_focus_on_show(data)` | 手动或自动初始聚焦 |
...
允许导航聚焦时（手柄，或 `keyboard_nav_focus` 打开后的键盘）：框架 Autoload **自动**处理方向键与 `ui_accept`；切到这类设备时按上次记录的导航项聚焦。鼠标/触屏不聚焦导航项。`_on_close()` 返回 `false` 可拦截关闭。
...
| `UNKNOWN` | 未纳入跟踪的物理输入及非物理事件（如 `InputEventAction`） |
...
| `input_should_hide_cursor(device_kind)` | 切换至该设备时是否应隐藏鼠标（读 `keyboard_nav_focus`） |
| `input_should_allow_nav_focus()` | 当前设备是否允许给导航项抢 GUI 焦点（见 `keyboard_nav_focus`） |

设备切换时按上表刷新 `Input.mouse_mode` 与导航项 `focus_mode`。不允许聚焦时导航项为 `FOCUS_NONE`，不抢焦点、不画环。
...
`pause_*` 控制 `get_tree().paused`；`ui_open` 的 suspend + dim 隔离多层 UI 输入；`ui_is_pause_open_blocked()` 避免 Esc 刚关 UI 后同帧误开其它 UI。
...
| `audio_set_paused` / `audio_is_paused` | 暂停 / 恢复 BGM、SFX（发出 `signal_gdframe_audio_paused`）；暂停期间 `play_bgm` / `play_sfx*` 不启动新播；`play_ui_sfx` 不受影响 |
...
**SFX 限流**：`max_per_bus` / `max_per_key` 为 `0` 时不限；池满可能抢轨。限流丢弃时 `play_sfx*` 返回 `false`（池未初始化、`audio_set_paused(true)` 等也会返回 `false`）。`audio_sfx_log_drops` 为 `true` 时限流丢弃会打日志（编辑器 `push_warning`，导出运行 `print`）。
...
| **UI管理** | 创建 `ui_*`、模糊搜索列表、编辑层 / 遮罩 / 是否参与导航、定位 / 打开 / 更名 / 删除（运行中禁止写操作） |
| **状态机** | 在 `FSM_ROOT` 创建/管理状态机与状态（**运行中禁止**增删改） |
| **对象池** | 运行中调试 `pool_stats`（F5 运行且调试器已连接时点 **刷新**） |
| **扩展管理** | 安装/更新可选扩展、创建扩展模块、**生成扩展 API** |
| **更新** | 检查 GDFrame 版本并安装；点 **更新** 前会确认（跨系列提示可能有兼容性变化）。成功后**自动重载插件**（运行中须先停止再手动点 **重载插件**）。已有 `config/config.tres` 会保留工程配置不覆盖；`config.gd` 仍随版本更新 |

`keyboard_nav_focus`、音频池等其余项在 **`config.tres` 检查器**中编辑。
